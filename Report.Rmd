...
The following code generates the visualization in figure \ref{fig:DowJonesHistoricVsDJIA}, which is a comparison with the actual Dow Jones Industrial Average index (highlighted in black).
...
```{r xxx, echo=TRUE, fig.cap="\\label{fig:xxx} Change rate against the Dow Jones Industrial Average index", fig.height=13, fig.width=10}
dow_jones_historical_records %>%
  filter(symbol != 'DIA') %>%
  left_join(
    dow_jones_historical_records %>%
...
```{r down_jones_correlation, echo=TRUE, fig.cap="\\label{fig:DowJonesCorrelation} Correlation among Dow Jones stocks", fig.height=10, fig.width=10}
dow_jones_historical_records %>%
