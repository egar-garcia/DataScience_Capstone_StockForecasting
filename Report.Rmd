...
A visualization of the generated training and test sets, which are being used to perform the evaluation of the methods/models, can be observed in figure \ref{fig:EvalSetsPlot} and it is plotted by the next code.
...
The mathematical equations to describe ARIMA models are fairly complex and beyond the scope of this report, furter details can be found on [@notation_arima_models]. In summary ARIMA has there are three important parameters to tune [@stock_prices_prediction]:
...
LSTM takes as predictors sequences of a given length (known as time steps) containing consecutive values in a time series and the outcome is the next value in the time sequence, i.e. the predictor is a subsequence of the time series instead of a single value. To prepare the training set only the values in the column close are needed, the date is not necessary since the important factor is the order. The preprocessing for this method consists in creating the subsequences with the closing prices previous to the date of the respective outcome. In addition, as recomended in most deep-learning approaches the values to predict (i.e. the closing prices) are scaled to the range from 0 to 1.
...
In contrast with other methods, a trainied LSTM model can take advance of an updated dataset to make further predictions, basically by constructing the subsequence used as predictor with updated data, the advantage is that the LSTM model can be kept in use without the need of training it again. For example and ideal case would be to train the LSTM model, then updating the dataset on daily bases and performing the prediction of the closing price for the next day.
...
Then in the next code, the predictions are performed for the data range given for the test set, in here is represented the case when the closing prices of the predicting range are completly unknown (p.e. when a long term prediction in the future is attempted).
...
The previoUs evaluation does not seem very encouraging, however the resuls are completly different when the prediction is performed when the dataset is kept updated, p.e. in the situation where the dataset is daily updated and the predictions performed for the next day. The following code simulates this scenario by providing an updated dataset to support the prediction process.
...
``````{r lstm_daily_eval_plot, echo=FALSE, fig.cap="\\label{fig:LSTMDailyEvalPlot} Prediction with LSTM doing daily update and predict", fig.height=6, fig.width=10}
plot_predictions(eval_sets, lstm_daily_predictions, lstm_train_predictions)
```

```{r lstm_daily_eval_result, results = 'asis', echo=FALSE}
knitr::kable(get_evaluation_results('LSTM - daily update/predict', lstm_daily_predictions),
             caption = '\\label{tab:LSTMDailyEval} Evaluation of prediction with LSTM doing daily update and predict')
```
...
The results of the evaluation for all the models tested in this report are gathered in table \ref{tab:Results}.

```{r results_table, results = 'asis', echo=FALSE}
knitr::kable(results %>% spread('Number of trading days ahead', 'RMSE'),
             caption = '\\label{tab:Results} Results of the evaluation')
```
