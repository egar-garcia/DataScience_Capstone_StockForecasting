...
Additionally, the problem is very different to other machine learning problems since the predictions should take place strictly in the future and not distributed along the domain of predictors/predictions. For this particular kind of problems the way to address them is by using techniques for time series forecasting. The tested classic machine learning problems (Linear Regression, GAM and SVM) are good to aproximate past observations but can not adapt to future tendencies, in contrast to the time series ones (ARIMA, Prophet and LSTM) that do a better job in that sense.
...
Finally, the objective of this project was accomplished by providing a set of constructions and models, which are able to be used in a RStudio or similar environment for doing experiments and analysis of data, targeting for them to be used for data scientists and/or (financial) data analysts.
