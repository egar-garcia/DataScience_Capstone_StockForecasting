...
In contrast with other methods, a trained LSTM model can take advance of an updated dataset to make further predictions, basically by constructing the sub-sequence used as predictor with updated data, the advantage is that the LSTM model can be kept in use without the need of training it again. For example and ideal case would be to train the LSTM model, then updating the dataset on daily bases and performing the prediction of the closing price for the next day.
...
In table \ref{tab:Results} are gathered the results of the evaluation for all the models tested in this report, the evaluation is based on the RMSE of their predictions against the test set's ground truth.
...
Another perspective of the result is provided by benchmarking the performance of the different machine learning methods implemented in this project, they are compared against the initial naive solution which is produced via Linear Regression.
...
The following code performs the benchmarking of the results against Linear Regression, and they are presented in table \ref{tab:ResultsBenchLR}.
...
knitr::kable(benchmark_against_linear_regression(results) %>%
  ...
  caption = '\\label{tab:ResultsBenchLR} Results of the evaluation benchmarked against Liner Regression')
```
...
Additionally, the problem is very different to other machine learning problems since the predictions should take place strictly in the future and not distributed along the domain of predictors/predictions. For this particular kind of problems the way to address them is by using techniques for time series forecasting.
...
Finally, the objective of this project was accomplished by providing a set of constructions and models, which are able to be used in a RStudio or similar environment for doing experiments and analysis of data, targeting it to be used for data scientists and/or (financial) data analysts.
