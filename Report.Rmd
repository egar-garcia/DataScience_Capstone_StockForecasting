...
In order to create the LSTM neural network for this model, Keras is being used. Keras is a high-level neural networks API capable of running on top of multiple back-ends like TensorFlow, CNTK or Theano. It was developed with a focus on enabling fast experimentation, being able to go from idea to result with the least possible delay is key to doing good research [@r_interface_keras]. Even though Keras is written in Python, the package `keras` provides an interface for using it with R.
...
The preprocessing mechanism for the prediction set complex as well, because LSTM takes as predictors sequences of a given length containing consecutive values in a time series, then the predicted value would be the next value in the time sequence. The preparation of the prediction set consist in creating subsequences containing the closing prices previous to the date to predict, normalized to a range from 0 to 1, since these are the inputs that the LSTM network accept.
